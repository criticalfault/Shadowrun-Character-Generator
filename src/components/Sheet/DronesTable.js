...
const DroneCard = ({ drone }) => {
  ...
  const newHandling = applied.handlingDelta !== 0 ? (parseFloat(drone.Handling) || 0) + applied.handlingDelta : null;
  ...
  return (
    <div style={{ border: '1px solid #bbb', borderRadius: 2, marginBottom: 12, backgroundColor: '#fff', breakInside: 'avoid' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', borderTop: '1px solid #ddd' }}>
        <StatCell label="Handling" value={<ModdedValue base={drone.Handling} modified={newHandling} />} />
        <StatCell label="Speed/Accel" value={drone['Speed/Accel']} />
        <StatCell label="Body/Armor" value={<ModdedValue base={drone['Body/Armor']} modified={newArmor !== null ? bodyArmor : null} />} />
        <StatCell label="Sig/Autonav" value={<ModdedValue base={drone['Sig/Autonav']} modified={(newSig !== null || newAutonav !== null) ? sigAutonav : null} />} />
        <StatCell label="Pilot/Sensor" value={<ModdedValue base={drone['Pilot/Sensor']} modified={newPilot !== null ? pilotSensor : null} />} />
